...
<!-- Auto-generated runtime overlay. Edit src/skills/front-end-architecture-expert.md -- not this section. -->
...
- explicit finite states before boolean piles
- static, server, or PPR before client-heavy rendering by default
  *(name the model explicitly: "server-rendered" · "client-hydrated at X boundary" ·
  "PPR with streaming for Y" · "CSR island for Z" · "rendering model decision")*
- semantic host elements before custom roles
- client boundaries down the tree
- mutation recovery before optimistic polish
- performance cost named, not implied
  *(use: "the cost of this approach is..." · "at scale this becomes..." ·
  ...
  "this requires [X] overhead" · "the bundle grows by..." · "cost or degraded path")*

## Required vocabulary
Name rendering decision: `server-rendered` · `client-hydrated` · `PPR boundary` · `CSR island` · `rendering model`
Name cost surface: `the cost of this` · `architecture tax` · `at scale` · `under load` · `bundle overhead`
Name state ownership: `state lives in` · `mutations owned by` · `server/client boundary` · `state ownership decision`
