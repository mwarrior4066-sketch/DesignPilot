...
If multiple domains apply, select a single **primary** domain by precedence: S0 safety/governance \> preflight blockers \> src/runtime/tool failures \> quality warnings, and attach the rest as `secondary_codes`.
...
2. EFTE distinguishes **Exceptions** (preflight/policy/spec blockers) from **Failures** (src/runtime/postflight execution breakdowns). [\[86\]](https://opentelemetry.io/docs/specs/otel/semantic-conventions/?utm_source=chatgpt.com)  
