...
def validate_load_state(pack_root: Path, route_id: str = '', task_id: str = '') -> Dict:
    statuses = []
    if not (pack_root / 'MASTER_CHAT_OPERATOR.md').exists():
        statuses.append({'class': 'below_minimum_viable_load', 'action': 'stop', 'detail': 'MASTER_CHAT_OPERATOR.md missing'})
    ...
    if not (pack_root / 'SESSION_CONTEXT.md').exists():
        if (pack_root / 'SESSION_CONTEXT_DEFAULTS.md').exists():
            statuses.append({'class': 'missing_session_state', 'action': 'continue_with_defaults', 'detail': 'SESSION_CONTEXT.md missing; defaults available'})
        else:
            statuses.append({'class': 'below_minimum_viable_load', 'action': 'stop', 'detail': 'SESSION_CONTEXT.md missing and defaults unavailable'})
...
def validate_runtime_overlay_assets(pack_root: Path) -> Dict:
    ...
    for rel in ['DEGRADED_MODE_PROTOCOL.md', 'VISUAL_INPUT_PROTOCOL.md', 'LIGHTWEIGHT_RESPONSE_PROTOCOL.md', 'QUICKSTART.md']:
        if not (pack_root / rel).exists():
            errors.append(f'{rel} missing')

    control_map = (pack_root / 'CONTROL_AUTHORITY_MAP.md').read_text(encoding='utf-8').lower()
