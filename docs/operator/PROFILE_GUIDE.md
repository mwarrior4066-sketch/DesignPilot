...
Use profile-only mode when the task is clearly inside one domain and the kernel would be unnecessary.

Use the full kernel for true cross-domain coordination, route conflict resolution, release-sensitive work, or proof-heavy decisions.

## CORE
General DesignPilot deployment for critique, planning, audit, and rebuild work inside one primary domain.
...
Escalate to the full kernel when more than one domain is central or proof sensitivity rises.
