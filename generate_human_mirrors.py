...
for task in contracts['tasks']:
    ...
    out.append('')
(ROOT / 'OUTPUT_CONTRACTS_BY_TASK.md').write_text('\n'.join(out), encoding='utf-8')
...
(ROOT / 'RUNTIME_VALIDATION_LAYER.md').write_text('\n'.join(rulebook), encoding='utf-8')
...
(ROOT / 'ROUTE_CATALOG.md').write_text('\n'.join(catalog), encoding='utf-8')
...
(ROOT / 'PACK_QUALITY_RUBRIC.md').write_text('\n'.join(rubric), encoding='utf-8')
